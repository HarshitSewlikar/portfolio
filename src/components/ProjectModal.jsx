    ...
    >
      ...
      <div className="relative card-elevated w-full sm:max-w-3xl max-h-[92vh] overflow-y-auto shadow-pop border-base-ink rounded-t-3xl sm:rounded-3xl animate-pop-in">
        ...
        <div className="p-5 sm:p-7 space-y-7">
          {/* Image */}
          <div className="rounded-2xl overflow-hidden border-2 border-base-ink aspect-[16/9] bg-accent-yellow-tint shadow-pop">
            <img
              src={project.image}
          ...
          <Section title="how i built it" text={project.implementation} />
          ...
          <Section title="what i learned" text={project.learning} />

          {project.team?.length ? (
            <Section
              title="team & guidance"
              text={`built with ${project.team.join(', ')}${
                project.guide ? `, guided by ${project.guide}` : ''
              }.`}
            />
          ) : null}
...
function Section({ title, text }) {
  ...
  return (
    <div>
      ...
      <p className="text-base-ink-soft text-sm leading-relaxed">{text}</p>
    </div>
  )
}
